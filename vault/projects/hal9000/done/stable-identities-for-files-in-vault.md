...
**Superseded (2026).** The vault **no longer** uses YAML front matter with UUID `id` and `references:`. Identity is the **normalized repo-relative path** to each note; relations between notes are **only** expressed as resolvable **links in the markdown body**. Tooling lives under [`.scripts/vault_fm`](../../../../.scripts/vault_fm/__init__.py) (`send`, `health`, link validation, rename-based link repair). Rationale and migration notes: [`../drop-frontmatter-as-dead-end.md`](drop-frontmatter-as-dead-end.md).
