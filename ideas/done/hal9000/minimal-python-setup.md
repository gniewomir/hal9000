...
7. **`send.sh`** — After version check, run Python over **staged** `.md` paths that **participate** (same path rules as health—see stable-identities doc). **No** staged `.md` at all → **warning, non-zero exit** (abort). **Staged only out-of-scope** (e.g. only root/`scripts/`) → **warning, exit 0**. On success, processed files are re-staged as above.
